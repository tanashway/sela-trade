...
export default function ContactsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Failed to send message. Please try again or email us directly at info@selatrade.com');
    } finally {
      setLoading(false);
    }
  };
              ...
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full py-4 bg-[#1a365d] hover:bg-[#2c5282] text-white font-semibold rounded-xl transition-all duration-300 hover:shadow-lg hover:shadow-[#1a365d]/30 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {loading ? (
                      <>
